...
export function serializePyqResponses(paper, answers = {}) {
  return (paper?.questions || []).map((question) => {
    ...
    const wasAnswered = answered(question, selectedAnswer);
    const grade = gradeQuestion(question, selectedAnswer);
    const wasCorrect = grade.status === 'correct';
    return {
      ...
      acceptedSets: question.acceptedSets || null,
      marksToAll: Boolean(question.mta),
      answered: wasAnswered,
