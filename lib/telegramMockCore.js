...
export function buildQuestionKeyboard(question, { sessionId, questionIndex, selected = [] }) {
  ...
  if (question.type === 'MSQ') {
    const submitLabel = selected.length ? `Submit selections (${selected.length} selected)` : 'Submit selections';
    rows.push([callbackButton(submitLabel, `tm:submit:${sessionId}:${questionIndex}`), skip]);
  } else {
    rows.push([skip]);
  }
